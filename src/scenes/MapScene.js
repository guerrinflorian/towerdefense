...
export class MapScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.loadingText = this.add.text(width / 2, height / 2, "Chargement des niveaux...", {
      fontFamily: "Orbitron",
      fontSize: "18px",
      color: "#9ae8ff",
    }).setOrigin(0.5);
  ...
  async hydrateData() {
    try {
      ...
      if (!this.currentChapter && chapterVMs.length > 0) {
        ...
        this.currentChapter = chapterVMs.find((c) => c.id === this.currentChapter.id) || chapterVMs[0];
      }
  ...
  addBackground(cx, height) {
    ...
    if (this.textures.exists("background_map")) {
      if (!this.bgImage) {
        this.bgImage = this.add.image(cx, height / 2, "background_map")
    ...
    if (!this.bgOverlay) {
      this.bgOverlay = this.add.rectangle(cx, height / 2, this.scale.width, height, 0x000000, 0.35);
      this.bgOverlay.setDepth(-0.5);
    } else {
      this.bgOverlay.setPosition(cx, height / 2).setSize(this.scale.width, height);
    }
  }

  addHeader() {
    const { width, height } = this.scale;
    this.title = this.add.text(width / 2, 50, "SÉLECTION DU SECTEUR", {
      fontFamily: "Impact",
      fontSize: "42px",
      color: "#fff",
    }).setOrigin(0.5).setShadow(0, 0, "#00f2ff", 20, true, true);

    this.chapterLabel = this.add.text(width / 2, 90, "", {
      fontFamily: "Orbitron",
      fontSize: "18px",
      color: "#9ae8ff",
    }).setOrigin(0.5);

    const backMain = this.add.text(width / 2, height - 40, "↩ RETOUR AU QG", {
      fontFamily: "Orbitron",
      fontSize: "16px",
      color: "#00f2ff",
    ...
    const backChapters = this.add.text(90, 50, "⟵ Chapitres", {
      fontFamily: "Orbitron",
      fontSize: "16px",
      color: "#9ae8ff",
    ...
    backChapters.on("pointerdown", () => this.scene.start("ChapterScene", { fromMainMenu: true }));
  ...
  updateChapterLabel() {
    ...
    const heartsInfo = hearts != null ? `• Pré-requis: ≤ ${hearts} cœurs sur le chapitre précédent` : "• Chapitre initial";
    this.chapterLabel.setText(`${this.currentChapter.name.toUpperCase()} ${heartsInfo ? `\n${heartsInfo}` : ""}`);
  }
  ...
  draw3DMap() {
    ...
    const gap = levels.length > 1 ? (width - margin * 2) / (levels.length - 1) : 0;
  ...
  create3DIsland(x, y, level, biomeType, isLocked) {
    ...
    const txtId = this.add.text(0, -12, level.id, { fontSize: "44px", fontFamily: "Impact", color: isLocked ? "#444" : "#fff" }).setOrigin(0.5);
    const txtName = this.add.text(0, 78, level.name.toUpperCase(), { fontSize: "11px", fontFamily: "Orbitron", color: isLocked ? "#555" : "#fff", fontWeight: "bold", align: "center", wordWrap: { width: 140 } }).setOrigin(0.5);
    ...
    this.levelIslands.set(level.id, { container, statsContainer, bestTime, bestHearts, isLocked });
    ...
    this.tweens.add({ targets: container, y: y - 15, duration: 2500 + (level.id * 200), yoyo: true, repeat: -1, ease: "Sine.easeInOut" });
  ...
  drawBridge(gfx, p1, p2, isLocked) {
    ...
    gfx.lineStyle(10, 0x000000, 0.2); curve.draw(gfx);
    gfx.lineStyle(3, color, isLocked ? 0.15 : 0.7); curve.draw(gfx);
  }
  ...
  showToast(message) {
    ...
    this.toast = this.add.text(width / 2, 140, message, {
      fontFamily: "Orbitron",
      fontSize: "14px",
      color: "#ffb3b3",
